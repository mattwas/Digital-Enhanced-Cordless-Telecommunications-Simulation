function [samples_tx,SamplingRate] = dect_modulate(packet_data,mod_scheme, mac_meta)
    ...
    switch mac_meta.Configuration
        ...
        case {"1b", "2", "2b", "3", "3b", "4a", "4b"}
            ...
            s_field_data_samples_mod = comm.DPSKModulator( ...
                    2, ...
                    pi/2,...
                    BitInput=1);
            s_field_data_samples = s_field_data_samples_mod(packet_data{1});


            a_field_data_samples_mod = comm.DPSKModulator( ...
                    2^mod_scheme.a_field_bits_per_symbol, ...
                    pi/(2^mod_scheme.a_field_bits_per_symbol),...
                    BitInput=1);
            a_field_data_samples = a_field_data_samples_mod(packet_data{2});
            

            if ~isequal(packet_data{3}, double.empty(0,1))
                b_z_field_data_samples_mod = comm.DPSKModulator( ...
                        2^mod_scheme.b_z_field_bits_per_symbol, ...
                        pi/(2^mod_scheme.b_z_field_bits_per_symbol),...
                        BitInput=1);
                b_z_field_data_samples = b_z_field_data_samples_mod(packet_data{3});
