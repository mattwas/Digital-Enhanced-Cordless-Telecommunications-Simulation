...
mac_meta.Oversampling = 8; % oversampling
...
synchronisation_toggle = 1;

tx = dect_tx(mac_meta);
rx = dect_rx(tx.mac_meta, synchronisation_toggle);
...
for l=1:numel(delay_spread)
    ...
    parfor i=1:num_of_workers
        ...
        for k = 1:1:num_of_packets_per_snr
            ...
            rayleighchan = comm.RayleighChannel( ...
                ...
                'PathDelays',path_delays*delay_spread(l), ...
    ...
    for k = 1:size(PER_b_field_cell,2)
        PER_b_field_array(1,k) = PER_b_field_cell{1,k};
    end
    
    PER_a_field_array = zeros(numel(delay_spread),size(PER_a_field_cell,2));
    
    for k = 1:size(PER_b_field_cell,2)
        PER_a_field_array(,k) = PER_a_field_cell{l,k};
    end
...
figure;semilogy(snr_db_vec_global,ber_a_field);
title("BER A-Field");
xlabel("SNR in dB");
