...
num_of_packets_per_snr =2.5e4;
snr_db_vec_global = 0 : 1 : 40;
num_of_workers = numel(snr_db_vec_global);
PER_a_field_cell = zeros(numel(delay_spread),numel(snr_db_vec_global));
PER_b_field_cell = zeros(numel(delay_spread),numel(snr_db_vec_global));
...
PER_b_field_array = zeros(numel(N_Rx),size(PER_b_field_cell,2));
PER_a_field_array = zeros(numel(N_Rx),size(PER_a_field_cell,2));
...
ber_b_field = cell(1,numel(N_Rx));
ber_a_field = cell(1,numel(N_Rx));
...
for l=1:numel(N_Rx)
    ...
    for k = 1:size(PER_b_field_cell,2)
        PER_b_field_array(l,k) = PER_b_field_cell(k);
    end
    
    
    for k = 1:size(PER_b_field_cell,2)
        PER_a_field_array(l,k) = PER_a_field_cell(k);
    end
...
figure;semilogy(snr_db_vec_global,PER_a_field_movmean(1,:));
title("PER A Field");
xlabel("SNR in dB");
grid on
hold on
semilogy(snr_db_vec_global,PER_a_field_movmean(2,:));
legend("N_{Rx} = 1", "N_{Rx} = 2");
xlim([0 40]);
ylim([10e-5 1]);
