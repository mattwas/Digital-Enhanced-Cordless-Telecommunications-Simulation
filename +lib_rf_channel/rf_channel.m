classdef rf_channel < handle
    ...
    % Channel Wrapper from Maxim, some minor adjustments have to be made
    ...
    properties
        ...
        samples_per_symbol      % this is needed for adding noise (replaced spectrum_occupied)
        ...
        %
        %   Noise is generated by using a local random number stream 'awgn_randomstream'.
        ...
        awgn_randomstream_seed;      % if awgn_random_source=local, this is the seed for the random stream (mt19937ar) for the awgn function         
    ...
    methods (Static = true, Access = public)
       ...
       function obj = rf_channel()
            ...
            obj.noise                   = [];
            obj.snr_db                  = [];
            obj.samples_per_symbol      = [];
            obj.N_TX                    = [];
            obj.N_RX                    = []; 
            
            obj.awgn_random_source      = [];
            obj.awgn_randomstream_seed  = [];
    ...
    methods (Static = false, Access = public)
        ...
        function [samples_antenna_rx] = pass_samples(obj, samples_antenna_tx, channel_time_in_seconds)
            ...
            if obj.noise == true
                ...
                if strcmp(obj.awgn_random_source,'global') == true
                    for i=1:1:obj.N_RX
                        awgn_chan = comm.AWGNChannel("NoiseMethod","Signal to noise ratio (SNR)",...
                                                     "SNR",obj.snr_db-obj.samples_per_symbol);
                        samples_antenna_rx(:,i) = awgn_chan(samples_antenna_rx(:,i));
                    end
                elseif strcmp(obj.awgn_random_source,'local') == true
                    for i=1:1:obj.N_RX
                        awgn_chan = comm.AWGNChannel("NoiseMethod","Signal to noise ratio (SNR)",...
                                                     "SNR",obj.snr_db-obj.samples_per_symbol,...
                                                     "RandomStream","mt19937ar with seed",...
                                                     "Seed",obj.awgn_randomstream_seed);
                        samples_antenna_rx(:,i) = awgn_chan(samples_antenna_rx(:,i));
                    end
                end
            end
        ...
        function [] = reset_random_awgn(obj)
          	if strcmp(obj.awgn_random_source, 'local') == true
                obj.awgn_randomstream_seed.reset();
            end
        end
    end
end
